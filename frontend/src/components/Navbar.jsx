  ...
  return (
    <nav className="navbar navbar-expand-lg campus-navbar sticky-top">
      <div className="container-fluid px-3 px-md-4">
        ...
        <button
          className="navbar-toggler border-0 shadow-none text-white opacity-75"
          type="button"
        ...
        <div className="collapse navbar-collapse" id="navbarContent">
          {!loading && user && isStaff && (
            <div className="navbar-nav me-auto ms-lg-4 d-flex gap-1 align-items-center">
              <span className="nav-link disabled text-white-50 small py-0 d-none d-lg-block">
                Operations
              </span>
              ...
              <NavLink to="/notifications" className={linkClass}>
                ...
                {unreadCount > 0 && (
                  <span
                    className="ms-1 badge rounded-pill bg-danger shadow-sm"
            ...
            {!loading && user && (
              <div className="d-flex align-items-center gap-2">
                <div className="text-end d-none d-sm-block">
                  <div
                    className="text-white fw-semibold lh-1"
                    style={{ fontSize: "0.9rem" }}
                  >
                    {user.fullName}
                  </div>
                  <div className="text-white-50 small lh-1 mt-1">
                    {user.role}
                  </div>
                </div>
                <Link
                  to="/settings"
                  className="rounded-circle bg-white text-primary fw-bold d-flex align-items-center justify-content-center text-decoration-none shadow-sm"
                  style={{
                    width: "38px",
                    height: "38px",
                    fontSize: "0.95rem",
                ...
                <button
                  className="btn btn-sm btn-outline-light ms-2 rounded-pill px-3"
                  onClick={logout}
                >
                  Logout
                </button>
